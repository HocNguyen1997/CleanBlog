const UserServices = require('../servives/user.sevices');
...
const login = async (req, res, next)=>{
    try{
        ...
        let checkLogin = await UserServices.checkLogin(userName);
...
const register = async (req, res, next)=>{
    try{
        ...
        let checkRegister = await UserServices.checkRegister(userName);
        // console.log(checkRegister.length);
        if(checkRegister.length > 0){
            res.status(200).json('Account already exists');
        } else {
            let item = await UserServices.registerAccount(userName, passWord);
            console.log(item)
